...
class VMSI():
    ...
    def initial_minimization(self):
        ...
        #x = res.x
        bounds = [(-np.inf, np.inf) for _ in range(2*x0.shape[0])] + [(0.001, 1000) for _ in range(x0.shape[0])]
        def linear_con(x):
            Aeq = np.concatenate((np.zeros(x0.shape[0]*2), np.ones(x0.shape[0])))
            E = np.dot(Aeq, x) - (np.mean(p0)*p0.shape[0])
            return E

        constraints = [{'type':'eq', 'fun':nonlinear_con, 'jac':nlc_jac},
                       {'type':'eq', 'fun':linear_con}]

        #res = minimize_ipopt(energy, np.ravel(x0, order='F'),jac=e_jac,bounds=bounds,constraints=constraints,options={'maxiter':5000}, tol=1e-8)
        #x = x.reshape(x0.shape, order='F')
    ...
    def fit(self):
        ...
        #theta_scale_factor = np.mean(np.abs(X0[:,2]))
        #q_scale_factor = np.mean(X0[:,0:2])
        #X0 = np.vstack([np.divide(X0[:,0:2], q_scale_factor).T, np.divide(X0[:,2], theta_scale_factor), X0[:,3]]).T
        ...
        beq = np.array([np.mean(X0[:,3]), np.mean(X0[:,2])])
        lincon = scipy.optimize.LinearConstraint(Aeq, lb=beq, ub=beq)

        constraints= [nonlincon, lincon]
        bounds = scipy.optimize.Bounds(lb=np.concatenate((-np.inf*np.ones(3*theta0.shape[0]), 0.001*np.ones(theta0.shape[0]))),
                                       ub=np.concatenate((np.inf*np.ones(3*theta0.shape[0]), 1000*np.ones(theta0.shape[0]))))

        def callback_fun(Xk,current_state):
            e = energy(Xk)
            print(str(current_state.nit) + '\t' + str(e) + '\n')
            return False
        ...
        res = scipy.optimize.minimize(energy,np.ravel(X0,order='F'),jac=e_jac,hess=e_hess,method='trust-constr',constraints=constraints,
                                      bounds=bounds,options={'maxiter':2000,'initial_tr_radius':np.sqrt(X0.size)},
                                      tol=1e-6, callback=callback_fun)
